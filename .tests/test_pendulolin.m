classdef test_pendulolin < matlab.unittest.TestCase
    properties
        base_fun
        base_jac
        test_fun
    ...
    methods 
        function obj = test_pendulolin()
            global folder_base
            global folder_user
            obj = obj@matlab.unittest.TestCase();
            old_folder = cd(folder_base);
            obj.base_fun = @funcpendulolin;
            obj.base_jac = @jacfuncpendulolin;
            cd(folder_user);
            obj.test_fun = @funcpendulolin;
            obj.test_jac = @jacfuncpendulolin;
            cd(old_folder);
        end
    end
